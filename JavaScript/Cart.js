...
const extraItems =
  JSON.parse(localStorage.getItem("extraItems")) || products.slice();
...
function updateCartPro() {
  ...
  cartProducts.forEach((cProduct, index) => {
    ...
    totalHTML.forEach((total, index) => {
      if (totalItem == 1) {
        total.innerHTML = `Subtotal (${totalItem} item): <b class="red">$${totalPrice.toFixed(
          2
        )}</b>`;
      } else {
        total.innerHTML = `Subtotal (${totalItem} items): <b class="red">$${totalPrice.toFixed(
          2
        )}</b>`;
      }
    });
...
function updateFilteredProducts() {
  filteredProducts = extraProducts.filter((eProduct) => {
    if (
      eProduct.name.toLowerCase().includes(searchBar.value.trim().toLowerCase())
    ) {
      return eProduct;
    }
  });
